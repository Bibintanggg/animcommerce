...
export default function Products() {
    ...
    const { data, isLoading, isFetching } = useQuery({
        queryKey: ["products", pagination.pageIndex, pagination.pageSize, debouncedSearch],
        queryFn: () =>
            getProducts(pagination.pageIndex + 1, pagination.pageSize, debouncedSearch),
    ...
    const stats = useMemo(() => {
        ...
        const published = list.filter((p) => p.is_active === "published" || (p as any).status === "published").length;
        const draft = list.filter((p) => p.is_active === "draft" || (p as any).status === "draft").length;
        const archived = list.filter((p) => p.is_active === "archived" || (p as any).status === "archived").length;
        const lowStock = list.filter((p) => Number(p.stock) <= 5).length;
        const totalStock = list.reduce((acc, p) => acc + (Number(p.stock) || 0), 0);
        const totalValue = list.reduce((acc, p) => acc + (Number(p.price) || 0) * (Number(p.stock) || 0), 0);
    ...
    // Dummy data buat chart (bisa diganti data real nanti)
        ...
        <div className="p-6 md:p-10 space-y-8">
            ...
            <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
                ...
                <CreateModal
                    ...
                    trigger={<Button className="gap-2 shadow-sm"><Package className="h-4 w-4" />Tambah Produk</Button>}
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                ...
                <div className="rounded-xl border bg-card p-5 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <p className="text-2xl font-bold tracking-tight text-emerald-600">{stats.published}</p>
                    <p className="text-[11px] text-muted-foreground mt-1">Aktif di toko</p>
                </div>
                ...
                <div className="rounded-xl border bg-card p-5 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <p className="text-2xl font-bold tracking-tight text-amber-600">{stats.draft}</p>
                    <p className="text-[11px] text-muted-foreground mt-1">Belum dipublish</p>
                </div>
                ...
                <div className="rounded-xl border bg-card p-5 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <p className="text-2xl font-bold tracking-tight text-rose-600">{stats.archived}</p>
                    <p className="text-[11px] text-muted-foreground mt-1">Tidak aktif</p>
                </div>

                <div className="rounded-xl border bg-card p-5 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-center justify-between mb-3">
                        <span className="text-xs font-medium text-muted-foreground">Stok Rendah</span>
                        <AlertTriangle className="h-4 w-4 text-orange-500" />
                    </div>
                    <p className="text-2xl font-bold tracking-tight text-orange-600">{stats.lowStock}</p>
                    <p className="text-[11px] text-muted-foreground mt-1">≤ 5 unit</p>
                </div>

                <div className="rounded-xl border bg-card p-5 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-center justify-between mb-3">
                        <span className="text-xs font-medium text-muted-foreground">Total Stok</span>
                        <Box className="h-4 w-4 text-violet-500" />
                    </div>
                    <p className="text-2xl font-bold tracking-tight">{stats.totalStock}</p>
                    <p className="text-[11px] text-muted-foreground mt-1">Unit tersedia</p>
                </div>

                <div className="rounded-xl border bg-card p-5 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-center justify-between mb-3">
                        <span className="text-xs font-medium text-muted-foreground">Nilai Stok</span>
                    ...
                    <p className="text-[11px] text-muted-foreground mt-1">Estimasi nilai</p>
                </div>

                <div className="rounded-xl border bg-card p-5 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-center justify-between mb-3">
                        <span className="text-xs font-medium text-muted-foreground">Rata-rata Harga</span>
            ...
            <div className="rounded-xl border bg-card p-5 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">7 hari terakhir (estimasi)</p>
                    </div>
                        ...
                        <DataTable
                            ...
                            columns={[
                                ...
                                },
                                {
                                    ...
                                    header: "Stok",
                                    render: (product) => (
                                        <span className={Number(product.stock) <= 5 ? "text-orange-600 font-medium" : ""}>
                                            {product.stock}
                                        </span>
                                    ),
                                },
                ...
                <aside className="space-y-4 xl:sticky xl:top-6">
                    <div className="rounded-xl border bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/40 dark:to-indigo-950/40 p-5 shadow-sm">
                        ...
                        <ul className="space-y-2.5 text-xs text-muted-foreground leading-relaxed">
                            <li className="flex gap-2">
                                ...
                                <span>Pastikan stok selalu ≥ 5 agar tidak muncul di daftar “Stok Rendah”.</span>
                            </li>
                            <li className="flex gap-2">
                                <span className="text-blue-500 mt-0.5">•</span>
                                <span>Gunakan status <strong>Draft</strong> dulu sebelum publish ke publik.</span>
                            </li>
                            <li className="flex gap-2">
                                <span className="text-blue-500 mt-0.5">•</span>
                                <span>Thumbnail berkualitas tinggi = produk lebih menarik di etalase.</span>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div className="rounded-xl border bg-card p-5 shadow-sm">
                        ...
                        <div className="space-y-2.5">
                            <div className="flex items-center gap-2.5 text-xs">
                                ...
                                <span className="text-muted-foreground">Published — tampil di toko</span>
                            </div>
                            <div className="flex items-center gap-2.5 text-xs">
                                <span className="h-2.5 w-2.5 rounded-full bg-slate-400" />
                                <span className="text-muted-foreground">Draft — masih disimpan</span>
                            </div>
                            <div className="flex items-center gap-2.5 text-xs">
                                <span className="h-2.5 w-2.5 rounded-full bg-rose-500" />
                                <span className="text-muted-foreground">Archived — tidak aktif</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="rounded-xl border border-dashed border-violet-300 dark:border-violet-700 bg-violet-50/50 dark:bg-violet-950/20 p-5">
                        ...
                        <p className="text-xs text-muted-foreground leading-relaxed">
                            Jaga produk agar selalu <strong>Published</strong> dan stok terisi.
                            Produk dengan stok rendah cenderung lebih sulit terjual.
                        </p>
                    </div>
                </aside>
            </div>
